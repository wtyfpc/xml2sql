...
//            }
            // 复制 resource 下的 06-resource-group.sql 文件到 parentDirectory
            try(InputStream resourceStream = getClass().getClassLoader()
                    .getResourceAsStream("sql/06-resource-group.sql")){
                if(resourceStream == null){
                    throw new IOException("Resource 06-resource-group.sql not found");
                }
                Path targetPath = parentDirectory.resolve("06-resource-group.sql");
                Files.copy(resourceStream, targetPath, StandardCopyOption.REPLACE_EXISTING);
            }
        }
    }
